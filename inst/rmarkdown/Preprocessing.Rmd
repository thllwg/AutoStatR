...
#section-explanationLime .content div{
  max-height: 300px;
  margin-top: 15px;
  padding: 0px !important;
}
#section-explanationLime .content table thead tr th{
  text-align: center !important;
}
#section-explanationLime .content table tbody tr td{
  text-align: center !important;
}
#section-explanationLime img{
  display: block;
  margin: 0 auto;
}
#section-explanationLimeTestData img{
  display: block;
  margin: 0 auto;
}
#section-explanationSample{
...
if(nrow(params$data_to_predict) > 0){
  cat(kable(prediction) %>% kable_styling(fixed_thead = T) %>% scroll_box(width = "100%"))
  cat('\n')
...
<p> To explain the model generated by tpot different model-agnostic methods are used, which are based on the book <i>Interpratable Machine Learning by Christoph Molnar</i>.</p>
...
<p>When analysing machine learning algorithms it is important to know, which features are most important for the prediction of the algorithm.</p>
...
<p>The goal of the first step was to analyse which features influence the prediction of the found model the most. As a next step it is analyzed, how the features influence the prediction of the model. This is done by accumulated local affects.</p>
...
for(i in 1:length(levels)){
  ...
  ale.desc[i] <- paste("The class ", "<b>", level, "</b>", " is based on the feature ", 
                       ...
                       temp.result[[1,"min"]], " and ", temp.result[[1,"max"]], ". ")
}
```
</div>
...
<h4>Lime</h4>
<p>The last step is to not look at an avereaged influence of the features, but at individual observation to explain, why the model makes an individual classification. This is especially useful when explaining the reason for individual predictions on the prediction data.</p>
...
<div id="explanationLimeTestData">
<h5>Explaining the predicted values</h5>
<p>For the predicted data Lime provides the below explanations. The colour represents the feature weight and gives an indication, which features and feature values are most responsible for the prediction.</p>
```{r echo=FALSE, results='asis'}
...
<p>To explain the inner working of the found model even further, randomely 5 observations are selected for each class and their belonging to each class is explained with LIME.</p>
...
for(level in levels){
   ...
   explanation <- lime::explain(tempdata2[,1:length(tempdata2)-1], explainer1, n_labels = 1, n_features = 5)
