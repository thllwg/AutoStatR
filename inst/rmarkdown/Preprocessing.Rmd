...
<p>The following two ALE-plots show how the two most important features (<b>`r paste(imp.features[1:2], sep = ", ")`</b>) influencec the prediction of the model:</p>
...
for(i in 1:length(levels)){
  level <- levels[i]
  ale.desc[i] <- paste("The class ", level, " is based on the feature ", 
                       first.feature, " most likely for values. ")
}
```
</div>

<p>For the given classification problem the most important feature according to the feature importance analysis  is <b>`r paste(imp.features[1], sep = ", ")`</b>. The ALE-plot provides an analysis how this feature influences the target <b>`r target`</b> with its `r length(levels)` classes. `r paste(ale.desc)`</p>
...
<p>The second most important feature according to the feature importance analysis is <b>`r paste(imp.features[2], sep = ", ")`</b>. The plot illustrates how <b>`r paste(imp.features[2], sep = ", ")`</b> influences the target <b>`r target`</b>. `r paste(ale.desc)`</p>

<h4>Lime</h4>
<p>The last step is to not look at an avereaged influence of the features, but at individual observation to explain, why the model makes an individual classification. This is especially usefull to explain the reason for individual predictions on the prediction data.</p>
