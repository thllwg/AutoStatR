...
<h1 class="title">`r paste('Automatic report for the dataset:',params$type)`</h1>
...
<p>The Automated Statistician has fitted a machine learning pipeline, that can predict the target variable <b>`r params$target`</b> in the dataset <b>`r params$title`</b> with an accurancy of <b>`r max(generations)`</b>. The best machine learning pipeline is:</p>
...
<p>The following plot shows the importance of each feature. The ratio of classification error is calculated as follows: Increase in classification error through permutation divided by the original classification error. The dot is the median after several permutations:</p>
...
p + labs(x = "Feature", y = "Feature Importance Ratio (Classification Error as Loss)") + theme_bw()
```
</div>

<p>The plot shows that from the <b>`r length(data) - 1`</b> features of the data set the following <b>`r length(imp.features)`</b> are identified as the most important features that influence the total prediction error the most: <b>`r paste(imp.features, sep = ", ")`</b>.</p>
