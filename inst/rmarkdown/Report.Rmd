...
<p>The Automated Statistician has fitted a machine learning pipeline, that can predict the target variable <b>`r params$target`</b> in the dataset <b>`r params$title`</b> with an classificationaccurancy of <b>`r max(generations)`</b>. The best machine learning pipeline is:</p>
